...
if to_read:
    dataframes = [pd.read_excel(f).drop(columns=[col for col in pd.read_excel(f, nrows=0).columns if col.startswith('Unnamed')], errors='ignore') for f in to_read]
...
if not st.session_state["df"].empty and "PRM" in st.session_state["df"].columns:
    selection_mapping = {f"{row['PRM']}": index for index, row in st.session_state["df"].iterrows()}
